// SUITS, RANKS, PAYTABLES, HAND_ORDERS, HAND_RANK, createDeck, evaluateHand,
// evaluateDeucesHand and evaluateWithVariant now live in js/hand-eval.js
// (loaded earlier in build.js, before this file) — this module still owns
// everything state-related: which variant is selected, current payouts,
// and unlock levels.
...
// createDeck() now lives in js/hand-eval.js.
...
// evaluateHand() and evaluateDeucesHand() now live in js/hand-eval.js.
// This wrapper just supplies this module's own gameVariant to the
// variant-explicit evaluateWithVariant() — every existing call site
// (evaluateForVariant(hand) in draw(), etc.) is unchanged.
function evaluateForVariant(h) {
    return evaluateWithVariant(h, gameVariant);
}
